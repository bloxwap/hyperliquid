...
import { type CValidatorActionParameters, CValidatorActionRequest } from "@bloxwap/hyperliquid/api/exchange";
...
const paramsSchema = valibotToJsonSchema(
  v.union(CValidatorActionRequest.entries.action.options.map((option) => v.omit(option, ["type"]))),
);
